...
int nega_scout_final(const board *b, const long long strt, bool skipped, int depth, int alpha, int beta){
    ...
    if (canput >= 2)
        sort(nb.begin(), nb.end(), move_ordering);
    int v, g;
    g = -nega_scout_final(&nb[0], strt, false, depth - 1, -beta, -alpha);
    if (g == inf)
        return -inf;
    if (beta <= g)
        return g;
    ...
    for (int i = 1; i < canput; ++i){
        g = -nega_alpha_ordering_final(&nb[i], strt, false, depth - 1, -alpha - window, -alpha);
        if (g == inf)
            return -inf;
        if (beta <= g)
            return g;
        if (alpha <= g){
            alpha = g;
            g = -nega_scout_final(&nb[i], strt, false, depth - 1, -beta, -alpha);
            if (g == inf)
                return -inf;
            if (beta <= g)
                return g;
