...
final_read = subprocess.Popen('./final_read.out'.split(), stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL)
...
def collect_data(num, s):
    ...
    while True:
        ...
        if turn >= 60 - 16:
            #print(rv.player)
            #print(grid_str)
            final_read.stdin.write((str(rv.player) + '\n' + grid_str).encode('utf-8'))
            final_read.stdin.flush()
            y, x = [int(elem) for elem in final_read.stdout.readline().decode().replace('\r\n', '').split()]
